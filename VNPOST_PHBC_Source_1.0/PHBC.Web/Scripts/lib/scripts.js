...
var array_width = [];
array_width['col-md-12'] = 100;
array_width['col-md-11'] = 91.66666667;
array_width['col-md-10'] = 83.33333333;
array_width['col-md-9'] = 75;
array_width['col-md-8'] = 66.66666667;
array_width['col-md-7'] = 58.33333333;
array_width['col-md-6'] = 50;
array_width['col-md-5'] = 41.66666667;
array_width['col-md-4'] = 33.33333333;
array_width['col-md-3'] = 25;
array_width['col-md-2'] = 16.66666667;
array_width['col-md-1'] = 8.33333333;
var lib = new Lib();
var popupdanhmuc;
function Lib() {
                    ...
                    window["search-time-" + idGuid] = setTimeout(function () {
                        ...
                        if (txt.length > 0) {
                            ...
                            tableSearch.find('tr').filter(function (index, element) {
                                $(this).attr('style', $(this).attr('valstyle'));
                            });
                        }
                    }, 300);
                });
    ...
    this.fromcontroler = function () {
        ...
        if (objDate) {
            ...
            if (objDate[0]) {//Neu co nhieu truong date
                objDate.each(function (i, elm) {
                    var valDate = $(elm).val();
                    valDate = valDate ? valDate.substr(0, 10) : "";
                    $(elm).val(valDate);
                });
            }
            else {//Neu co 1 truong date
        ...
        $('.form-group').find('.label-checkbox > input:checkbox').unbind("click");
        $('.form-group').find('.label-checkbox > input:checkbox').each(function () {
            var objInput = $(this).parent('.label-checkbox').next('div').children('input,select,textarea');
            if (objInput == null) return;
            if ($(this).prop('checked')) {
                objInput.removeAttr('disabled');
                objInput.attr('required', 'required');
            }
            else {
        ...
        $('.form-group').find('.label-checkbox > input:checkbox').click(function () {
            var objInput =  $(this).parent('.label-checkbox').next('div').children('input,select,textarea');
            if(objInput == null) return;
            if ($(this).prop('checked')) {
                objInput.removeAttr('disabled');
        ...
        $('.form-group').find('.control-label + div > input:checkbox').each(function () {
            ...
            $(this).parent().css({ 'right': 'calc(' + array_width[$(this).parent().attr('class')] + '% - 50px)' }).attr('checkbox', '').parent().attr('panelcheckbox', '');
        });
    ...
    this.removeDisabled = function (fDisable) {
        if (fDisable) {
            $('.form-group').find('.label-checkbox > input:checkbox').each(function () {
                var objInput = $(this).parent('.label-checkbox').next('div').children('input,select,textarea');
                if (objInput == null) return;
            ...
            $('.form-group').find('.label-checkbox > input:checkbox').each(function () {
                $(this).parent('.label-checkbox').next('div').children('input,select,textarea').removeAttr('disabled');
            });
        }
    }
    //function chi su dung voi id va popupdanhmuc
    this.bindvalueforlib = function (objid, objtext, valueid, valuename) {
        console.log(objid + " " + " " + objtext + " " + valueid + " " + valuename);
        $("#" + objid).val(valueid);
        $("#" + objtext).val(valuename);
        popupdanhmuc.close();
    }
}