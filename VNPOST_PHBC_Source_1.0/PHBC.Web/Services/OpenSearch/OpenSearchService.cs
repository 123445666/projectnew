namespace PHBC.Web.Services
{
    ...
    public sealed class OpenSearchService : IOpenSearchService
    {
        ...
        public string GetOpenSearchXml()
        {
            ...
            string searchUrl = this.urlHelper.AbsoluteRouteUrl(ControllerRoute.HomeGetSearch, new { query = "{searchTerms}" });
            // The link to the page with the search form on it. The home page has the search form on it.
            string searchFormUrl = this.urlHelper.AbsoluteRouteUrl(ControllerRoute.HomeGetIndex);
