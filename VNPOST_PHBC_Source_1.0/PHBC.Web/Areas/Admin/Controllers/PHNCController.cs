...
namespace PHBC.Web.Areas.Admin.Controllers
{
    ...
    /// Author : vietvb
    /// </summary>
    public class PHNCController : BaseController
    {
        //private DB_PHBCEntities db = new DB_PHBCEntities();
        IBPHNCBussiness db;
        public PHNCController(IBPHNCBussiness _iBPHNCBussiness)
        {
            db = _iBPHNCBussiness;
            ViewBag.TitleName = " Phân hướng nhu cầu ";
        }
        // GET: Admin/PHNC
        ...
        public ActionResult Edit([Bind(Include = "Id,UnitCode,ThongTinBaoId,DiemTiepNhanId,CreateDate,CreateBy")] BPhanHuongNhuCau bPhanHuongNhuCau, string listId, string typeid)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                Session[Application.Session.UnitModel] = objUnitNew;
                buildMessage("Bạn đã sửa thiết lập thành công .");
